...
import time
import datetime
from pystudy.sysutils.re_utils import filter_tags
def get_news_list():
    with open('./data_ali/ali.json', 'r', encoding='utf8')as fp:
        json_data = json.load(fp)
    news_list =  get_ali_data(json_data)
    return news_list

today = datetime.datetime.now().strftime('%Y-%m-%d')
def get_ali_data(data):
    """
    获取疫情防控信息
    :param data:
    :return:
    ...
    news_list = []
    for news in data:
        date_str = news['publishTime']
        ltime = time.localtime(int(date_str)/1000)
        timeStr = time.strftime("%Y-%m-%d %H:%M:%S", ltime)
        if today == timeStr[0:10]:
            prevent_news = News(timeStr, news['title'], filter_tags(news['articleAbstract']), news['url'],'阿里健康','实时资讯','')
            news_list.append(prevent_news)
    return news_list
import xlwt
...
if __name__ == '__main__':
    ...
    data_write("./阿里健康"+today+".xls", news_list)
