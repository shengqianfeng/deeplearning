...
import time
def get_news_list():
    url = 'https://file1.dxycdn.com/2020/0130/492/3393874921745912795-115.json'
    headers = {
        "User-agent": choice(user_agent)
    }
    response = requests.get(url, headers=headers)
    news_list = get_dxys_data(response)
    return news_list

import datetime

today = datetime.datetime.now().strftime('%Y-%m-%d')
def get_dxys_data(response):
    ...
    if resp_json['code'] == 'success':
        ...
        for resp_data in resp_list:
            ltime = time.localtime(resp_data['pubDate'] / 1000)
            timeStr = time.strftime("%Y-%m-%d %H:%M:%S", ltime)
            # 只爬取今天的新闻
            if today==timeStr[0:10]:
                news = News(timeStr, resp_data['title'], resp_data['summary'], resp_data['sourceUrl'],'丁香医生','','')
...
if __name__ == '__main__':
    ...
    data_write("./丁香医生"+today+".xls",news_list)
