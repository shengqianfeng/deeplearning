...
def get_news_list():
    ...
    news_list = get_people_data(html)
    return news_list

import datetime
today = datetime.datetime.now().strftime('%m-%d')
def get_people_data(html):
    html.encoding = 'GB2312'
    soup = BeautifulSoup(html.text, 'html.parser')
    dtnews = soup.find_all('div', attrs={"class": "dtnews"})
    news_list = []
    for dtnew in dtnews:
        if 'http://' not in dtnew.find('a')['href']:
            content_url = 'http://society.people.com.cn/' + dtnew.find('a')['href']
        else:
            content_url= dtnew.find('a')['href']
...
if __name__ == '__main__':
    ...
    data_write("./人民日报"+today+".xls",news_list)
