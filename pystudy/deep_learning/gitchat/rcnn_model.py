...
class rcnn(object):
    ...
    def __init__(self,sess,model_output_path):
        # 设定 RNN 模型的参数
        self.epochs = 30
        self.batch_size = 250
    ...
    def build_model(self,y_output,embedding_output):
        ...
        '''
        self.accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(logits_out, 1), tf.cast(y_output, tf.int64)), tf.float32))
    ...
    def train_model(self,test_dict,x_train,y_train,x_data,y_output):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(num_batches):
                ...
                train_dict = {x_data: x_train_batch, y_output: y_train_batch, self.dropout_keep_prob: 0.5}
    ...
    def predict(self,embedding_output,input):
        ...
        其中参数 shape 表示生成的张量的维度是 [rnn_size, 2]， mean 均值默认为 0， stddev 标准差设置为 0.1。
        '''
        reader = pywrap_tensorflow.NewCheckpointReader(self.model_output_path)
        bias = reader.get_tensor('biases/Variable')
        weight = reader.get_tensor('weights/Variable')
        # logits 是这个全连接层的输出，作为 softmax 的输入，在接下来定义损失函数时用到
        logits_out = tf.matmul(last, weight) + bias
        return logits_out
